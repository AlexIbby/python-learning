...
#get course info
with open(course_information, "r") as course_file:

    course_file = course_file.readlines()

    course_name = course_file[0].replace("name: ","").strip()
    
    study_credits = course_file[1].replace("study credits: ","")
    study_credits = f"{study_credits} credits".strip()
    
    header = f"{course_name}, {study_credits}"
...
with open("results.txt", "w") as file:
    file.write(header)
    file.write(f"\n{header_underline}")
    file.write(f"\n{name_header:<30}{exec_nbr_header:<10}{exec_pts_header:<10}{exm_pts_header:<10}{tot_pts_header:<10}{grade_header}")
    for student in student_records:
        name = student_records[student]['name']
        exec_nbr = student_records[student]['score']
        exec_pts = student_records[student]['exec_pts']
        ...
        file.write(f"\n{name:<30}{exec_nbr:<10}{exec_pts:<10}{exam_pts:<10}{tot_pts_print:<10}{grades_print}")
...
with open("results.csv","w") as results_file:
    for index, student in enumerate(student_records):
        ...
        #grade
        grade = grades_print = student_records[student]['grade']
        grade = str(grade)
        grade = grade.strip()


        if index == 0:
            results_file.write(f"{id};{name};{grade}")
